...
export default function AdminDashboard() {
  ...
  const handleEditImageChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    setEditing((prev) =>
      prev ? { ...prev, newFile: file, previewUrl: URL.createObjectURL(file) } : prev,
    );
  };

  const handleSaveEdit = async () => {
    if (!editing) return;
    if (!editing.nameTh.trim() || !editing.nameEn.trim() || !editing.nameCn.trim() || !editing.link.trim()) {
      return;
    }
    ...
    try {
      ...
      await updatePlace(selectedStation, editing.id, {
        name: { th: editing.nameTh.trim(), en: editing.nameEn.trim(), cn: editing.nameCn.trim() },
        img: imgUrl,
  ...
  const inputCls = "border border-[#C6C6C6] rounded-xl px-3 py-2 text-[13px] text-[#543A14] outline-none placeholder:text-[#C6C6C6] w-full";
  ...
  return (
    <div className="min-h-screen bg-[#FBFCF0] p-5">
      <div className="max-w-lg mx-auto">
        ...
        <div className="mb-5">
          <label className="block text-[13px] text-[#8B724E] font-medium mb-1">เลือก Station</label>
          <select
            value={selectedStation}
            onChange={(e) => { setSelectedStation(e.target.value); setEditing(null); }}
            className="w-full border border-[#C6C6C6] bg-white rounded-xl px-4 py-2.5 text-[14px] text-[#543A14] outline-none"
          >
            {STATION_IDS.map((id) => (
              <option key={id} value={id}>{STATION_NAMES_TH[id]} ({id})</option>
            ))}
          </select>
        </div>
        ...
        <div className="bg-white rounded-2xl border border-[#D9D9D9] shadow-sm p-4 mb-5">
          ...
          <div className="flex flex-col gap-y-2.5">
            <input placeholder="ชื่อสถานที่ (ภาษาไทย) *" value={form.nameTh}
              onChange={(e) => setForm((f) => ({ ...f, nameTh: e.target.value }))} className={inputCls} />
            <input placeholder="Place name (English) *" value={form.nameEn}
              onChange={(e) => setForm((f) => ({ ...f, nameEn: e.target.value }))} className={inputCls} />
            <input placeholder="地点名称 (中文) *" value={form.nameCn}
              onChange={(e) => setForm((f) => ({ ...f, nameCn: e.target.value }))} className={inputCls} />
            <input placeholder="ลิงก์ (URL) *" value={form.link}
              onChange={(e) => setForm((f) => ({ ...f, link: e.target.value }))} className={inputCls} />
            ...
            {uploadProgress !== null && !editing && (
              <div className="w-full bg-[#F5F5F5] rounded-full h-2">
                <div className="bg-[#BF4B17] h-2 rounded-full transition-all duration-300" style={{ width: `${uploadProgress}%` }} />
              </div>
            )}
            ...
            <button onClick={handleAdd} disabled={saving}
              ...
              {saving ? (uploadProgress !== null ? `กำลังอัพโหลด ${uploadProgress}%` : "กำลังบันทึก...") : "+ เพิ่มสถานที่"}
            </button>
          </div>
        </div>
          ...
          {loading ? (
            ...
            <div className="flex flex-col gap-y-3">
              {places.map((place) =>
                editing?.id === place.id ? (
                  // Edit form inline
                  <div key={place.id} className="bg-white rounded-2xl border border-[#BF4B17] shadow-sm p-4 flex flex-col gap-y-2.5">
                    <p className="text-[12px] font-semibold text-[#BF4B17]">แก้ไขสถานที่</p>
                    <input placeholder="ชื่อ (ไทย) *" value={editing.nameTh}
                      onChange={(e) => setEditing((s) => s && { ...s, nameTh: e.target.value })} className={inputCls} />
                    <input placeholder="Name (English) *" value={editing.nameEn}
                      onChange={(e) => setEditing((s) => s && { ...s, nameEn: e.target.value })} className={inputCls} />
                    <input placeholder="名称 (中文) *" value={editing.nameCn}
                      onChange={(e) => setEditing((s) => s && { ...s, nameCn: e.target.value })} className={inputCls} />
                    <input placeholder="ลิงก์ (URL) *" value={editing.link}
                      onChange={(e) => setEditing((s) => s && { ...s, link: e.target.value })} className={inputCls} />
                    ...
                    <button type="button" onClick={() => editFileInputRef.current?.click()}
                      ...
                      {editing.newFile ? editing.newFile.name : "เปลี่ยนรูปภาพ (ไม่บังคับ)"}
                    </button>

                    <div className="w-full h-28 rounded-xl overflow-hidden">
                      <img src={editing.previewUrl ?? editing.img} alt="preview" className="w-full h-full object-cover" />
                    </div>
                    ...
                    <div className="flex gap-x-2">
                      <button onClick={handleSaveEdit} disabled={editing.saving}
                      ...
                      <button onClick={() => { setEditing(null); setUploadProgress(null); }}
                  ...
                  // Normal card
                  <div key={place.id} className="bg-white rounded-2xl border border-[#D9D9D9] shadow-sm p-3 flex gap-x-3 items-start">
                    ...
                    <div className="flex flex-col gap-y-1 shrink-0">
                      <button onClick={() => startEdit(place)} className="text-[12px] text-[#543A14] font-medium">แก้ไข</button>
                      <button onClick={() => handleDelete(place.id)} className="text-[12px] text-red-400 font-medium">ลบ</button>
                    </div>
                  </div>
                )
              )}
