...
export default function AdminKome() {
  ...
  const setEditName =
    (lang: "th" | "en" | "cn") => (e: React.ChangeEvent<HTMLInputElement>) =>
      setEditing(
        (s) => s && { ...s, name: { ...s.name, [lang]: e.target.value } },
      );
    ...
    <div className="flex flex-col gap-y-4">
      <div className="bg-white rounded-2xl border border-[#D9D9D9] shadow-sm p-4">
        ...
        <div className="flex flex-col gap-y-2">
          <input
            placeholder={p("form.groupName", "inTh", true)}
            value={form.name.th}
            onChange={setName("th")}
            className={inputCls}
          />
          <input
            placeholder={p("form.groupName", "inEn", true)}
            value={form.name.en}
            onChange={setName("en")}
            className={inputCls}
          />
          <input
            placeholder={p("form.groupName", "inCn", true)}
            value={form.name.cn}
            onChange={setName("cn")}
            className={inputCls}
          />
        ...
        items.map((item) =>
          editing?.id === item.id ? (
            ...
            >
              ...
              <input
                placeholder={p("form.groupName", "inTh")}
                value={editing.name.th}
                onChange={setEditName("th")}
                className={inputCls}
              />
              <input
                placeholder={p("form.groupName", "inEn")}
                value={editing.name.en}
                onChange={setEditName("en")}
                className={inputCls}
              />
              <input
                placeholder={p("form.groupName", "inCn")}
                value={editing.name.cn}
                onChange={setEditName("cn")}
                className={inputCls}
              />
              <input
                value={editing.phone}
