...
export default function AdminPopup() {
  ...
  const handleSave = async () => {
    if (!header.th.trim() || !header.en.trim() || !header.cn.trim()) {
      setError("กรุณากรอกหัวข้อครบทั้ง 3 ภาษา");
      return;
    }
    setError("");
    setSaving(true);
    try {
      ...
      await updateStationPopup(selectedStation, { header, desc, img: finalImg });
      setImageFile(null);
      setPreviewUrl(null);
      refetch();
    } catch (e) {
      setError(`เกิดข้อผิดพลาด: ${e instanceof Error ? e.message : String(e)}`);
    } finally {
      setSaving(false);
    }
  };
      ...
      <div className="bg-white rounded-2xl border border-[#D9D9D9] shadow-sm p-4">
        ...
        {loading ? (
          ...
          <div className="flex flex-col gap-y-2">
            <p className="text-[12px] text-[#8B724E] font-medium">{t("form.title")}</p>
            <input placeholder={p("form.title", "inTh")} value={header.th} onChange={(e) => setHeader((h) => ({ ...h, th: e.target.value }))} className={inputCls} />
            <input placeholder={p("form.title", "inEn")} value={header.en} onChange={(e) => setHeader((h) => ({ ...h, en: e.target.value }))} className={inputCls} />
            <input placeholder={p("form.title", "inCn")} value={header.cn} onChange={(e) => setHeader((h) => ({ ...h, cn: e.target.value }))} className={inputCls} />

            <p className="text-[12px] text-[#8B724E] font-medium mt-1">{t("form.descFull")}</p>
            <textarea placeholder={p("form.descFull", "inTh")} value={desc.th} onChange={(e) => setDesc((d) => ({ ...d, th: e.target.value }))} rows={3} className={`${inputCls} resize-none`} />
            <textarea placeholder={p("form.descFull", "inEn")} value={desc.en} onChange={(e) => setDesc((d) => ({ ...d, en: e.target.value }))} rows={3} className={`${inputCls} resize-none`} />
            <textarea placeholder={p("form.descFull", "inCn")} value={desc.cn} onChange={(e) => setDesc((d) => ({ ...d, cn: e.target.value }))} rows={3} className={`${inputCls} resize-none`} />

            <p className="text-[12px] text-[#8B724E] font-medium mt-1">{t("dashboard.image")}</p>
            {(previewUrl || imgUrl) && (
              <img
            ...
            >
              {imgUrl || previewUrl ? t("dashboard.changeImage") : t("dashboard.selectImage")}
            </button>
