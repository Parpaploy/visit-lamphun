...
export default function NavbarMenuBtn({
  ...
  return (
    <button
      ...
      className={`${location.pathname === path ? "bg-[#BF4B17] text-white font-semibold" : "font-normal bg-[#FEF0DB] text-[#543A14]"} w-full flex justify-start items-center gap-3 rounded-full p-1 pr-1.5 wrap-break-word`}
    >
      ...
      <div className="flex flex-col justify-center items-start">
        <p className="text-[16px] text-start">{title}</p>
        {desc && (
          <p className="text-[12px] text-[#757575] font-normal -mt-1">{desc}</p>
        )}
      </div>
